...
(
    ...
    output [32-1:0]                                         tstamp_req_fingerprint,
        
    //Fingerprint AVST Output
    output                                                aso_fingerprint_valid,
    output [32-1:0]                                        aso_fingerprint,
    ...
    reg    [32-1:0]    fingerprint;
    reg    [32-1:0]    fingerprint_r;
    reg            aso_fingerprint_valid_r;
    wire               reset_fp;

    assign reset_fp = &(fingerprint[FP_WIDTH-1:0]);
...
//    assign tstamp_req_valid = aso_pktout_ready & aso_pktout_valid & aso_pktout_sop;
    ...
    begin
        ...
        begin
            if (reset_fp & (aso_pktout_ready == 1'b1 & aso_pktout_valid == 1'b1 & aso_pktout_sop == 1'b1)) begin
                fingerprint   <= 'd0;
                fingerprint_r <= fingerprint;
            end   
            else if(aso_pktout_ready == 1'b1 & aso_pktout_valid == 1'b1 & aso_pktout_sop == 1'b1)
            begin
